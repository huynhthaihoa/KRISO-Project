...
def freezing_graph(sess, graph_file, output_node_names):
    ...
    tf.compat.v1.train.write_graph(sess.graph, directory, base, as_text=False)
    print('Done')

    print('>> Saving `{}`... '.format(ckpt), end='')
    tf.compat.v1.train.Saver().save(sess, ckpt, write_meta_graph=False)
    print('Done')
...
def main():
    ...
    model_out = model(inputdata=tf.compat.v1.placeholder(tf.float32, [1, image_height, image_width, 1]))

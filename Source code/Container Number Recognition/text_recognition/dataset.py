...
class Dataset():
    ...
    def __init__(self, annotation_path, image_width, image_height, batch_size=1, shuffle=False, repeat=None):
        ...
        dataset = dataset.map(
            lambda filename, label: tuple(
                tf.compat.v1.py_func(self.read_py_function, [filename, label], [tf.float32, tf.string])))
        dataset = dataset.map(
            lambda image, label: tuple(
                tf.compat.v1.py_func(self.convert_labels_to_int32_array, [image, label],
                           [tf.float32, tf.int32])))
        dataset = dataset.map(
    ...
    def parse_datasets_arg(annotation_path):
        ...
        for annpath in annotation_path.split(','):
            ...
            annotation_folder = os.path.join("/home/kotomi/path/to/clone/text_recognition",annotation_folder)

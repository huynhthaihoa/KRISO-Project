...
recognition_model_h_path = "recognition_h/model_all.ckpt-8000"
#recognition_model_h_path = "/home/blin/Downloads/recognition_v/model_all.ckpt-90000"
recognition_model_v_path = "recognition_v/model_all.ckpt-146000"
...
images_ph_h = tf.compat.v1.placeholder(tf.float32, [bs, 32, 240, 1])
model_out_h = model(inputdata=images_ph_h)
saver_h = tf.compat.v1.train.Saver()
sess_r_h=tf.compat.v1.Session()
...
images_ph_v = tf.compat.v1.placeholder(tf.float32, [bs, 32, 320, 1])
model_out_v = model(inputdata=images_ph_v)
saver_v = tf.compat.v1.train.Saver()
sess_r_v=tf.compat.v1.Session()
...
impaths = glob('//192.168.0.199/ubay_share/Container data/container image/*.JPG')
config = {}
...
for impath in impaths:
    ...
    try:
        ...
        #t3 = time.time()
    #print('recognition_time: ', (t3-t2),'result', predicted)
        line = 'Image name: {0}. Code {1}. Processing time: {2}s'.format(imname, predicted, time.time() - t1)
        #print('Image name: {0}. Code {1}. Processing time: {2}s'.format(imname, predicted_r, time.time() - t1))#imname, i, predicted_r, time.time() - t1)
        #line = imname + ':' + predicted_r + '\n'
    except:
        line = "Image name: {0}. Cannot determine the code. Processing time: {1}s".format(imname, time.time() - t1)
        #print("Image name: {0}. Cannot determine the code. Processing time: {1}s".format(imname, time.time() - t1))
        #line
    print(line)
    res_txt.write(line)
    i+=1
...
print('total_time: {0}, average_time: {1}'.format((total_time2 - total_time1), (total_time2 - total_time1) / i))

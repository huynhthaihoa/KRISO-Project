...
with tf.compat.v1.Graph().as_default():
    recogniton_graph_def = tf.compat.v1.GraphDef()
...
for impath in impaths:
    ...
    try:
        #recognition
        predicted_r = recognition(im, sess_r_h, sess_r_v , bboxs, (240, 32), input_h, input_v, model_out_h, model_out_v, decoded_h, decoded_v)
        
        #post-processing
        #predicted_r = revise(predicted)
        #t3 = time.time()
    #print('recognition_time: ', (t3-t2),'result', predicted)
        line = 'Image name: {0}. Code: {1}. Processing time: {2}s\n'.format(imname, predicted_r, time.time() - t1)
        ...
        line = "Image name: {0}. Code: unidentified. Processing time: {1}s\n".format(imname, time.time() - t1)
        #print("Image name: {0}. Cannot determine the code. Processing time: {1}s".format(imname, time.time() - t1))
        #line
    print(line)
    #cv2.putText(im, line, (0, 0), cv2.QT_FONT_NORMAL, 1, (0, 255, 0) , 1)
